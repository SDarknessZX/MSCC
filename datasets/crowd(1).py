...
def cal_new_tensor(img_tensor, keypoints,min_size=256):
    w,h = img_tensor.size
    if min(h, w) < min_size:
        ratio_h, ratio_w = min_size / h, min_size / w
        keypoints[:,1]=ratio_h*keypoints[:,1]
        keypoints[:,0]=ratio_w*keypoints[:,0]
        if ratio_h >= ratio_w:
            img_tensor =img_tensor.resize((min_size, int(min_size / h * w)),Image.BILINEAR)
        else:
            img_tensor = img_tensor.resize((int(min_size / w * h), min_size),Image.BILINEAR)
...
class Base(data.Dataset):
    ...
    def train_transform(self, img, keypoints):
        ...
        if st_size < self.c_size:
            img,keypoints=cal_new_tensor(img,keypoints,self.c_size)
        wd, ht = img.size
...
class Crowd_cc50(Base):
    def __init__(self, root_path, crop_size,
        ...
        super().__init__(root_path, crop_size, downsample_ratio)
        self.method = method
        val_range=range(fold*10-9,fold*10)
        self.im_list = sorted(glob(os.path.join(self.root_path, '*.jpg')))
        # self.im_list=[]
        #
        # if method=="train":
        #     for i in self.im_list1:
        #         if int(i.split('/')[-1].split('.')[0]) in val_range:
        #             continue
        #         else:
        #             self.im_list.append(i)
        #
        # else:
        #     for i in self.im_list1:
        #         if int(i.split('/')[-1].split('.')[0]) in val_range:
...
class Crowd_sh(Base):
    def __init__(self, root_path, crop_size,
        ...
        if method not in ['train', 'val']:
            raise Exception("not implement")
        if method=="train":
            self.im_list = sorted(glob(os.path.join(self.root_path, 'images', '*.jpg')))+sorted(glob(os.path.join(self.root_path.replace('train','test'), 'images', '*.jpg')))
        else:
